...
#define VDATE "2019.02.20"          //Sketch Version Date
...
// #'s are miscopied characters ... ~ are European characters
...
// design custom characters by Hans, ON4CDU

byte A_umlaut[8] =   {B01010, B00000, B01110, B10001, B11111, B10001, B10001, B00000}; // 'Ä' . - . - (26)
byte O_umlaut[8] =   {B01010, B00000, B01110, B10001, B10001, B10001, B01110, B00000}; // 'Ö' - - - . (17)
byte U_umlaut[8] =   {B01010, B00000, B10001, B10001, B10001, B10001, B01110, B00000}; // 'Ü' . . - - (28)
byte A_grave[8]  =   {B01000, B00100, B01110, B10001, B11111, B10001, B10001, B00000}; // 'À' . - - . - (50)
byte E_aigu[8]   =   {B00100, B01000, B01110, B10000, B11110, B10000, B11110, B00000}; // 'É' . . - . . (59)
byte C_cedille[8]=   {B01110, B10001, B10000, B10000, B10001, B01110, B00010, B00110}; // 'Ç' -.-.. (43)
...
//send dadididadah (-..--) to change sidetone pitch
  lcd.createChar(6,didit); 
  lcd.createChar(7,dahdah);
// define special charaters
  lcd.createChar(0, A_umlaut); //     German
  lcd.createChar(1, O_umlaut); //     German
  lcd.createChar(2, U_umlaut); //     German
  lcd.createChar(3, A_grave);  //     French
  lcd.createChar(4, E_aigu);   //     French
  lcd.createChar(5, C_cedille);//     French
...
void printCharacter() {           
  ...
  if (lcdGuy == '~') {    // European Character Set
    switch (myNum) {
      case 26:
        lcdGuy = 0; // Ä
        break;
